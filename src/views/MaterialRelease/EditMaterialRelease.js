...
import { MaterialRelease_Insert, GetPODetailsLIkeSearch, GetGRNByGRNIdForReleasedMaterials, GetBOMMaterialsQty, GetExpiryDatesforMaterialRelease, GetMaterialforRelease, GetBatchNoDetails, BatchNo_Update, GetMaterialforReleaseForEdit, ReleasedMaterial_Delete, ReleasedMaterialMaster_Update } from './MaterialReleaseService'
...
function EditMaterialRelease() {
    ...
    const [EditDsable, setEditDsable] = useState(false)
    const [vBatchNoDisable, setvBatchNoDisable] = useState(false)
    ...
    const getPOByPOId = () => {
        GetGRNByGRNIdForReleasedMaterials(nGRNId).then(res => {
            ...
            setBomDetail(res.GRNMaster[0].vBOMName)
            setnBOMUnit(res.GRNMaster[0].nBOMUnit)
    ...
    const getBOMMaterialsQty_2 = (BId, MId) => {
        GetBOMMaterialsQty(BId, MId).then(res => {
    ...
    const deleteItem = (GRNId, MId, ExpDate, QTYOut) => {
        confirmAlert({
            title: 'Alert !!',
            closeOnClickOutside: false,
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        ...
                        ReleasedMaterial_Delete(data).then(res => {
                            if (res) {
            ...
            buttons: [
                {
                    label: 'Yes',
                    onClick: () => {
                        setEditFirstFrom(true)
                        setdateandactiveDisable(false)
                    },
                },
                {
                    label: 'No',
                    ...
                    onClick: () => {
                        ...
                        ReleasedMaterialMaster_Update(data).then(res => {
                            if (res) {
                                ...
                                setvBatchNoDisable(true)
                                setUpdatebtn(false)
                                setEditFirstFrom(false)
                                setdateandactiveDisable(true)
                            }
                        })
                    },
                },
        ...
        <div className='citymasterContainer'>
            <div className='dateFilter-2'>
                <div className='displayflexend mt-2'>
                    ...
                    <Box className='inputBox-1' style={{position: 'relative'}}>
                        <FormControl fullWidth className='input'>
                            <TextField
                            sx={muiStyles.input}
                                value={vBatchNo}
                        ...
                        {updatebtn == false ?
                            <div className='updatebtnbox'>
                                <button className='updatebtn' onClick={batchnoupdate}>Edit</button>
                            </div>
                            :
                            <div className='updatebtnbox'>
                                <button className='updatebtn' onClick={updateBatchNo}>Update</button>
                            </div>
                    ...
                    <FormGroup >
                        <FormControlLabel style={{marginRight:0}} control={<Checkbox checked={btActive} value={btActive} onChange={e => setBtActive(e.target.checked)} />} label="Active" disabled={dateandactiveDisable} />
                    </FormGroup>
                    ...
                    {editFirstFrom == true ?
                        <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end',gap:10 }}>
                            <button onClick={updateDateandbtActive} className='addbtn' title='BOM Info'> Update </button>
                            <button title='Clear' className='addbtn' onClick={disableDateandactive}><ReplayIcon fontSize='large' /></button>
                        </div>
                        :
                        <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end' }}>
                            <button onClick={openDateAndActive} className='addbtn' title='BOM Info'> Edit </button>
                        </div>
                    }
                    </div>
                    {TableShow == true ?
